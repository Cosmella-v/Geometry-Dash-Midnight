...
class $modify(GameObjectFactoryExt, GameObject) {
    bool hasBeenActivatedByPlayer(PlayerObject* player) {
        THIS_CONF;
        bool active = false;
        if (config) if (config->m_hasbeenActivatedBypass) { 
            if (config->m_hasbeenActivated) goto Act;
            return active;
        };
        active = GameObject::hasBeenActivatedByPlayer(player);
        if (active) return active;
        if (config) { 
            if (config->m_hasbeenActivated) {
                goto Act;
                Act:
                  active = config->m_hasbeenActivated(player);
                  return active;
            }
        }
        return active;
